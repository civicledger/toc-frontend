    ...
    <>
                  ...
                  <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                    ...
                    <Formik
                      ...
                      onSubmit={(values, actions) => {
                        strategyService
                          .create({ ...values, companyId: company.id })
                          .then(({ data }) => {
                            history.push(`/strategies/${data.id}`);
                            actions.resetForm();
                            setOpen(false);
                          })
                          .catch(error => {
                          ...
                          <Form>
                            <div className="mt-3">
                              ...
                              <div className="mb-5">
                                ...
                                {!props.errors.name && <p className="text-gray-600 text-sm mt-1 mx-2">Provide a name to identify your strategy</p>}
                                <ErrorMessage component="p" name="name" className="text-red-500 text-sm mx-2" />
                              </div>
                              <div className="mb-5">
                                <label htmlFor="vision" className="block font-medium text-gray-700 mb-1">
                                  Vision
                                </label>
                                <Field as="textarea" type="textarea" name="vision" />
                                {!props.errors.vision && (
                                  <p className="text-gray-600 text-sm mt-1 mx-2">Define the vision of this strategy for anyone reading</p>
                                )}
                                <ErrorMessage component="p" name="vision" className="text-red-500 text-sm mx-2" />
                              </div>
                            </div>
                            <hr />
                            <div className="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse">
                              ...
                              >
                                <PlusCircleIcon className="w-5 inline-block mr-2" /> Create Strategy
                              </button>
                              <button
                                type="button"
      ...
      <div>
        <button className="mt-3 bg-indigo-500 p-2 px-4 text-white rounded hover:bg-indigo-600" onClick={() => setOpen(true)}>
          <PlusCircleIcon className="w-5 inline-block mr-2" /> Create new Strategy
        </button>
      </div>
    </>
  );
